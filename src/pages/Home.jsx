    ...
    <section ref={ref} className="block w-full py-24 bg-white dark:bg-black relative overflow-hidden">
      ...
      <div className="section-container relative">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <span className="section-label block mb-6" style={anim(inView, 0)}>{t.homeAbout.label}</span>
            ...
            <p className="text-slate-600 dark:text-gray-300 text-base leading-relaxed mb-6" style={anim(inView, 0.3)}>
              {t.homeAbout.body} <strong className="text-slate-900 dark:text-white">{t.homeAbout.bodyStrong}</strong> {t.homeAbout.bodyEnd}
            </p>
            <div className="flex gap-10 mb-10" style={anim(inView, 0.4)}>
            ...
            <div style={anim(inView, 0.5)}>
              <Link to="/haqqimizda"><button className="btn-outline">{t.homeAbout.cta}</button></Link>
            </div>
          </div>

          <div style={anim(inView, 0.3, 'right', 0.8)} className="bg-primary/10 border border-primary/30 p-10 relative">
    ...
    <section ref={ref} className="block w-full py-24 bg-slate-50 dark:bg-[#050505] relative">
      <div className="section-container">
        <div className="flex flex-col lg:flex-row justify-between items-end mb-16 gap-6">
          <div>
            <span className="section-label block mb-4" style={anim(inView, 0)}>{t.homeServices.label}</span>
            <h2
              className="font-display text-4xl lg:text-6xl font-black text-slate-900 dark:text-white"
              style={{ lineHeight: 1.25, overflow: 'visible', ...anim(inView, 0.1, 'bottom', 0.7) }}
            >
              {t.homeServices.heading1} <span className="text-accent italic">{t.homeServices.heading2}</span>
            </h2>
          </div>
          <div style={anim(inView, 0.4)}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {serviceItems.map((s, i) => (
            <div
              key={s.number}
              style={anim(inView, 0.1 + i * 0.1)}
              className="border border-slate-200 dark:border-white/10 p-7 hover:border-primary/40 hover:bg-primary/5 transition-colors duration-300 group bg-white dark:bg-transparent"
            >
  ...
  return (
    <section ref={ref} className="block w-full py-24 bg-white dark:bg-black relative">
      <div className="section-container">
        <div className="flex flex-col lg:flex-row justify-between items-end mb-16 gap-6">
          <div>
            <span className="section-label block mb-4" style={anim(inView, 0)}>{t.homePortfolio.label}</span>
            <h2
              className="font-display text-4xl lg:text-6xl font-black text-slate-900 dark:text-white"
              style={{ lineHeight: 1.25, overflow: 'visible', ...anim(inView, 0.1, 'bottom', 0.7) }}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {projects.map((p, i) => (
            <div key={p.id} style={anim(inView, 0.1 + i * 0.1)} className="group cursor-pointer overflow-hidden border border-slate-200 dark:border-white/10 hover:border-primary/30 transition-colors duration-300">
...
function HomeContactCTA() {
  ...
  return (
    <section ref={ref} className="block w-full py-24 bg-white dark:bg-black relative">
      <div className="section-container">
        <div style={anim(inView, 0, 'bottom', 0.8)} className="bg-primary/10 border border-primary/30 p-12 lg:p-20 text-center relative overflow-hidden">
          ...
          <p className="text-slate-600 dark:text-gray-300 text-base leading-relaxed max-w-lg mx-auto mb-10">{t.homeCta.body}</p>
          <Link to="/elaqe">
            <button className="btn-primary text-base px-10 py-5">{t.homeCta.cta}</button>
          </Link>
        </div>
      </div>
    </section>
